...
## Document
I have completed the task. And supporting different screen sizes (responsive).
I had add "start": "webpack-dev-server --hot" in package.json. 
Run app: cmd -> go to folder -> enter cmd: npm run start or npm start. -> url: localhost:3000
Run server: cmd -> go to folder -> enter cmd: npm run server. 
Test App: -> 